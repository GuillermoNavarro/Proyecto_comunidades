...
function GestionUsuarios({ usuario }) {
  ...
  const btnBaja = async (usuario) => {
    if (window.confirm(`¿Seguro que quiere dar de baja a ${usuario.nombre} ${usuario.apellidos}?`,)) {
      try {
  ...
  const btnAlta = async(usuario) => {
    if (window.confirm(`¿Seguro que quiere reactivar a ${usuario.nombre} ${usuario.apellidos}?`,)) {
      try {
